...
<script type="text/javascript">
  ...
  if ($('adjuntar<?php echo($transaccion); ?>')) {
    $('adjuntar<?php echo($transaccion); ?>').addEvent('click', function(e) {
      var upload = new File.Upload({
        ...
        data: {transaccion: '<?php echo($transaccion); ?>', proveedor: '<?php echo($_REQUEST['proveedor']); ?>', categoria: $('categoria').value, observacion: $('observacion').value},
        archivos: ['archivo'],
  ...
  function terminar<?php echo($transaccion); ?>() {
    MUI.Proveedores_Proveedor_Consultar('<?php echo($_REQUEST['proveedor']); ?>');
